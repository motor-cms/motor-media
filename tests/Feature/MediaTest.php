...
describe('File', function () {
    it('can get all Files')
        ...
        ->get('/api/files')
        ->assertStatus(200)
        ->assertJson(fn (AssertableJson $json) => $json->has('data', 10, fn (AssertableJson $data) => $data->has('id')
            ->has('author')
            ->has('source')
            ->etc())
            ->etc());
    ...
    it('can get a specific File', fn () => $this->asAdmin()
        ->get('/api/files/'.File::first()->id)
        ->assertStatus(200)
        ->assertJson(fn (AssertableJson $json) => $json->has('data', fn (AssertableJson $data) => $data->has('id')
            ->has('description')
            ->has('author')
            ->has('source')
            ->has('is_global')
    ...
    it('can delete files', function () {
        $filecount = File::count();
        $this->asAdmin()
            ->delete('/api/files/'.File::first()->id)
            ->assertStatus(200);
        expect($filecount - File::count())->toBe(1);
    });

    it("can't do anything without permissions", function () {
        $this->asBasic()
            ->getJson('/api/files')
            ->assertStatus(403);
        $this->asBasic()
            ->getJson('/api/files/'.File::first()->id)
            ->assertStatus(403);
        $this->asBasic()
            ->post('/api/files', [])
            ->assertStatus(403);
